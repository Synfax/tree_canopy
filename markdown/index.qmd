...
rents_public_plot <- ggplot(sa2_prices %>% filter(property_type == 'HOUSE'), mapping = aes(x = median_band, y = public_percentage, text = public_percentage)) + 
...
mb = mesh_blocks %>%
  ...
  rename(sa1_code_2021 = 'SA1_MAIN16') 

# sa1_summary_old <- agg_df_filtered %>%
#   group_by(sa1) %>%
...
sa1_summary <- coverage(type = 'overall', group = 'sa1_code_2021') %>%
  left_join(
  coverage(exp = street_exp, type = 'street', group = 'sa1_code_2021')
  , by = 'sa1_code_2021') %>%
  left_join(
    coverage(exp = resi_exp, type = 'residential', group = 'sa1_code_2021')
  , by = 'sa1_code_2021') %>%
  left_join(mb, by = 'sa1_code_2021') %>%
  left_join(sa1_sf, by = 'sa1_code_2021')
...
uhi_plot <- ggplot(sa1_summary %>% filter(CHG_FLAG21 == 0), aes(x = street_percentage, y = uhi, text = paste("Street tree coverage: ", scales::percent(street_percentage, accuracy = 1), "<br>UHI effect: ", round(uhi, digits = 2), "°C"))) +  
  geom_point(color = "#006D2C") + 
  labs(
...
# fixest::etable(feols(uhi ~ street_percentage, data = sa1_summary, vcov = 'hetero') , feols(uhi ~ street_percentage + overall_percentage, data = sa1_summary, vcov = 'hetero') , feols(uhi ~ street_percentage + overall_percentage + residential_percentage, data = sa1_summary, vcov = 'hetero'))

#library(fixest)

#feols(uhi ~ street_percentage, data = sa1_summary)
  ...
  reactable(
    ...
    columns = list(
      ...
      coverage_per_sqm = colDef(
        show = TRUE, 
        name = "Average coverage on lots suitable for upzoning (%)",
        footer = NA,
        footerStyle = list(fontWeight = "bold")
      ),

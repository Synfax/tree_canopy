...
int_comp <- ggplot(big_df, mapping = aes(distance, cov, group = city)) +
  xlab("Distance to Centre (m)") +
  ylab("Mean Tree Coverage (%)") +
  labs(
    title = "Tree coverage of the most liveable cities (2024)",
    color = "City"
  ) +
...
build_plotly <- function(chart, data) {
  ...
  for(i in seq_along(p$x$data)) {
    p$x$data[[i]]$text <- paste0(
      ...
      "Percentage: ", sprintf("%.2f%%", p$x$data[[i]]$y)
    )
...
p1 <- build_plotly(stacked_chart, combined_longer)
p2 <- build_plotly(stacked_chart_fill, combined_longer)
```
...
sa2_public_plot_house <- ggplot(sa2_prices %>% filter(property_type == 'HOUSE'), mapping = aes(x = arithmetic_mean, y = public_percentage, text = sa2_name, stroke = NA, fill = within15), axe) + xlab('Average Detached House Price ($m)') + ylab(' Tree Coverage of Public Areas (%)') + geom_point() + labs(caption = "Source: Neosec") +  theme_yimby

sa2_public_plot_unit <- ggplot(sa2_prices %>% filter(property_type == 'UNIT'), mapping = aes(x = arithmetic_mean, y = public_percentage, text = sa2_name, stroke = NA, fill = within15), axe) + xlab('Average Unit Price ($m)') + ylab(' Tree Coverage of Public Areas (%)') + geom_point() + labs(caption = "Source: Neosec") +  theme_yimby
...
#ggplot(house_price_data, mapping = aes(x = average_house_price, y = residential_percentage, color = direction, text = SAL_NAME21), axe) + xlab('Average House Price ($m)') + ylab('Residential Area Tree Coverage (%)') + geom_point() + scale_color_manual(name = "direction", values = colours) + theme_yimby

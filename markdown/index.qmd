...
<table>
  ...
  <tr>
    <td><strong>Invest $1840 per dwelling in public greening</strong></td>
  </tr>
  <tr>
    <td><strong>Grow 136 MCGs of new tree canopy</strong></td>
  </tr>
</table>
```
...
trees_grouped_sa1 <- coverage(group = grouping, df = agg_df_filtered) %>%
  left_join(grouping_sf, by = (grouping)) %>%
  st_set_geometry('geometry') %>%
  rowwise() %>%
  mutate(tree_percentage = round(tree_percentage, 1)) %>%
...
#write_sf(trees_grouped_sa1, '../exports/sa1_trees.shp', append = F)
...
sd_df = SharedData$new(as.data.frame(trees_grouped@data), group = sd_map$groupName())
  ...
  maplibre(style = mapgl::carto_style('positron'), center = c(145.063115,-37.814175), zoom = 9.5, pitch = 40) %>%
  ...
  #write_sf(trees_grouped_sa1, '../exports/sa1_tree_coverage.csv')
...
write_csv(big_df %>% as.data.frame(), '../exports/international_comparisons.csv')
...
write_csv(combined_longer %>% as.data.frame(), '../exports/stacked_bar_chart.csv')
...
write_csv(combined_df %>% dplyr::select(c(lga_name_2022, street_percentage, residential_percentage, reserve_percentage)) %>% as.data.frame(), '../exports/coverage_type_table.csv')
...
write_csv(seifa_sa2 %>% as.data.frame(), '../exports/seifa_sa2_scores.csv')
...
filtered_sa2_prices %>%  write_csv('../exports/sa2_house_unit_prices_plot.csv')
...
write_csv(sa2_prices %>% as.data.frame(),'../exports/sa2_rents_plot.csv')
...
write_csv(tt %>% as.data.frame(),'../exports/urban_heat_island.csv')
...
target_df %>%
  reactable(theme = sandstone(),
            ...
            columns = list(
              ...
              initial_coverage = colDef(show = F),
              coverage_per_sqm = colDef(show = T, name = "Current total tree canopy coverage (%)"),
              lost_coverage = colDef(name = "Public canopy planted per year (Ha)"),
              final_coverage = colDef(name = "Future total public canopy (Ha)"),
              percentage_change = colDef(name = "Growth in public canopy coverage (%)"),
              home_target = colDef(name = paste0(years,'-year housing target'), align = "right")
            ))

target_df %>% write_csv(.,'../exports/MM_table.csv')
...
target_df %>% write_csv(.,'../exports/GOV_table.csv')

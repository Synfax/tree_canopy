...
#| echo: false
coverage(exp = expression(!(zone_short %in% c('roads','other'))),group = 'zone_short', df = agg_df_filtered) %>%
...
lga_bplot <-  agg_df_filtered %>% filter( !(zone_short %in% c('roads','other')) ) %>%
...
**Next:** [Measuring Melbourne's street tree canopy](/model.html) 